    ...
    def shoppingOffers(self, price: List[int], specials: List[List[int]], needs: List[int]) -> int:
        ...
        def dfs(needs: Tuple[int]):
            ...
            for special in specials:
                # 如果买了这个 special，那么接下来还需要多少
                next_needs = tuple(needs[i] - special[i] for i in range(len(needs)))
                if min(next_needs) >= 0:
                    best = min(
                        best,
                        dfs(next_needs) + special[-1],
                    )
            # 如果一个 special 都不买，那么要花多少钱
            best = min(best, sum(price[i] * need for i, need in enumerate(needs)))
            return best
